...
weights = {
    ...
    'wc6': tf.get_variable("W6", shape=[25088, 3072], initializer=tf.contrib.layers.xavier_initializer()),
    'wc7': tf.get_variable("W7", shape=[3072, 384], initializer=tf.contrib.layers.xavier_initializer()),
    'wc8': tf.get_variable("W8", shape=[384, 48], initializer=tf.contrib.layers.xavier_initializer()),
    'wc9': tf.get_variable("W9", shape=[48, 6], initializer=tf.contrib.layers.xavier_initializer())
}
...
biases = {
    ...
    'bc6': tf.Variable(tf.random_normal([3072])),
    'bc7': tf.Variable(tf.random_normal([384])),
    'bc8': tf.Variable(tf.random_normal([48])),
...
with g.as_default():
    ...
    while True:
        if len(testDatasetNameArray) < i * 20:
            break
